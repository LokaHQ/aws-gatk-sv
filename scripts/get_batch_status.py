...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run Batch Status. This can take multiple arguments."
    )
    parser.add_argument(
        "-p",
        "--aws_profile",
    ...
    parser.add_argument(
        ...
        dest="cloudwatch_log_group_name",
        required=False,
        default=None,
    ...
    parser.add_argument(
        "-i",
        "--cloudwatch_query_interval",
        dest="cloudwatch_query_interval",
        required=False,
        default=600,
        type=int,
    ...
    return (
        ...
        args.cloudwatch_log_group_name,
        args.cloudwatch_query_interval,
    )
...
def get_batch_job_id_list():
    ...
    epoch_batch_start = (
        time.mktime(datetime.datetime.fromisoformat(batch_start_time).timetuple())
        * 1000
    )
    ...
    for job_status in status_needed:
        ...
        response = batch_client.list_jobs(
            jobQueue=batch_job_queue, jobStatus=job_status, maxResults=123
        )
        ...
        try:
            ...
            if job_status == "RUNNING":
                container_instance_arns = [
                    response["jobs"][0]["container"]["containerInstanceArn"].split("/")[
                        -1
                    ]
                ]
                ...
                container_instance_arns = list(
                    set(
                        [
                            i["container"]["containerInstanceArn"].split("/")[-1]
            ...
            for i in response["jobs"][0]["attempts"]:
                if "reason" in str(i):
                    reasons = (
                        i["container"]["reason"]
                        if not reasons
                        else reasons + "|" + i["container"]["reason"]
                    )
            ...
            final_list.append(
                {
                    ...
                    "stopped_at": response["jobs"][0]["stoppedAt"]
                    if "stoppedAt" in response["jobs"][0]
                    else None,
                    "image": response["jobs"][0]["container"]["image"],
                    "vcpus": response["jobs"][0]["container"]["vcpus"]
                    if "vcpus" in str(response["jobs"][0]["container"])
                    else response["jobs"][0]["container"]["resourceRequirements"][0][
                        "value"
                    ],
                    "memory": response["jobs"][0]["container"]["memory"]
                    if "memory" in str(response["jobs"][0]["container"])
                    else response["jobs"][0]["container"]["resourceRequirements"][1][
                        "value"
                    ],
            ...
            logger.warning(
                "The job id : %s failed parsing with response: %s" % (job_id, response)
            )
...
def get_resource_usage():
    ...
    epoch_start = int(
        datetime.datetime.strptime(batch_start_time, "%Y-%m-%d %H:%M:%S").timestamp()
    )
    ...
    logger.info(
        "Starting to query Cloud Watch Logs Group with an Interval : %s from start time : %s"
        % (cloudwatch_query_interval, batch_start_time)
    )
    ...
    try:
        ...
        logger.error(
            "The cloudwatch log parsing has failed. Kindly check.", exc_info=True
        )
    ...
    if len(final_df) > 0:
        final_df = (
            final_df.groupby(["task_id", "container_instance_id"]).mean().reset_index()
        )
        final_df.to_csv("%s/task_resource_usage.csv" % output_path, index=False)

    logger.info(
        "The number of tasks for which resource usage was gathered is as : %s"
        % len(final_df)
    )
...
def get_summaries(job_details_df):
    ...
    """
    job_modules_df = pd.read_csv(
        "%s/../configs/job_names_and_modules.csv" % current_script_dir
    )

    # Sub Module Level Summary
    logger.info("Considering only Successful Jobs")
    success_job_details_df = job_details_df[
        job_details_df["job_status"].isin(["SUCCEEDED"])
    ]
    temp_df = success_job_details_df[["job_name", "started_at", "stopped_at"]]
    temp_df["avg_duration_across_all_jobs"] = pd.to_datetime(
        temp_df["stopped_at"], infer_datetime_format=True
    ) - pd.to_datetime(temp_df["started_at"], infer_datetime_format=True)
    temp_df["avg_duration_across_all_jobs"] = temp_df[
        "avg_duration_across_all_jobs"
    ] / np.timedelta64(1, "h")
    temp_df = pd.merge(temp_df, job_modules_df, on="job_name", how="left")
    submodule_summary_df = (
        temp_df.groupby(["module_name", "module_number", "main_module_name"])
        .agg(
            {
                "started_at": np.min,
                "stopped_at": np.max,
                "avg_duration_across_all_jobs": np.average,
            }
        )
        .reset_index()
    )
    submodule_summary_df = pd.merge(
        submodule_summary_df,
        temp_df.value_counts(
            ["main_module_name", "module_name", "module_number"]
        ).reset_index(name="job_counts"),
        on="module_name",
        how="left",
    )
    submodule_summary_df["duration"] = pd.to_datetime(
        submodule_summary_df["stopped_at"], infer_datetime_format=True
    ) - pd.to_datetime(submodule_summary_df["started_at"], infer_datetime_format=True)
    submodule_summary_df["duration"] = submodule_summary_df[
        "duration"
    ] / np.timedelta64(1, "h")
    submodule_summary_df = submodule_summary_df[
        [
            "module_number_x",
            "main_module_name_x",
            "module_name",
    ...
    ].sort_values(["module_number_x", "started_at"])
    ...
    highlevel_summary_df = (
        submodule_summary_df.groupby(["module_number_x", "main_module_name_x"])
        .agg({"started_at": "min", "stopped_at": "max", "job_counts": "sum"})
        .reset_index()
    )
    highlevel_summary_df["duration"] = (
        pd.to_datetime(highlevel_summary_df["stopped_at"])
        - pd.to_datetime(highlevel_summary_df["started_at"])
    ).dt.total_seconds()
    highlevel_summary_df["duration"] = pd.to_datetime(
        highlevel_summary_df["duration"], unit="s"
    ).dt.strftime("%H:%M:%S")
    ...
    if job_id_list:
        ...
        if len(batch_job_details_df) > 0 and cloudwatch_log_group_name:
            ...
            if len(container_resource_consumption_df) > 0:
                # Join the DFs
                output_df = pd.merge(
                    batch_job_details_df,
                    container_resource_consumption_df,
                    on="task_id",
                    how="left",
                )
                print(output_df)
        ...
        output_df.to_csv(
            "%s/%s_job_details.csv" % (output_path, batch_job_queue.replace("-", "_")),
            index=False,
        )
